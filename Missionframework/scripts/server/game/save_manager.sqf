...
if (!isNil "_saveData") then {
    ...
    private _object = objNull;
    {
        ...
        if ((toLower _class) in KPLIB_classnamesToSave) then {
			...
			// If we have cargo saved, restore
            if (!KP_liberation_clear_cargo && (count _cargo > 0)) then {
				...
				{
					_object addItemCargoGlobal [_x, ((_items select 1) select ((_items select 0) find _x))];
				} count (_items select 0);
				private _magazines = _cargo select 1;
				{
					_object addMagazineCargoGlobal [_x, ((_magazines select 1) select ((_magazines select 0) find _x))];
				} count (_magazines select 0);
				private _weapons = _cargo select 2;
				{
					_object addWeaponCargoGlobal [_x, ((_weapons select 1) select ((_weapons select 0) find _x))];
				} count (_weapons select 0);
